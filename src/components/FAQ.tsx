...
function FAQ({ question,key,answer }: Props) {
  ...
  return (
    <div className="w-full border rounded-lg px-2 py-3 flex flex-col items-center justify-between mt-4 cursor-pointer"    onClick={() => handleToggle(key)}>
      <div className="flex justify-between items-center w-full">
      <h1>{question}</h1>
      <span className="w-6 aspect-square bg-black rounded-full flex items-center justify-center">
        <TiMediaPlay className={`text-white ${openId===key ? "rotate-90 transition-all duration-300 ease-in-out" :"transition-all duration-300 ease-in-out"}`} />
      </span>
      </div>
      <div
